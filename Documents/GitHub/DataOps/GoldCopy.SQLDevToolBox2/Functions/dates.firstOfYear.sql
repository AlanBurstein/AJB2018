...
--==== Basic use ->
  ...
  SELECT f.yearStart FROM dates.firstOfYear(@date,  0) f; -- first of this year
  SELECT f.yearStart FROM dates.firstOfYear(@date,  1) f; -- first of next year
  SELECT f.yearStart FROM dates.firstOfYear(@date, -1) f; -- first of last year
...
BEGIN
  ...
  FROM @table t
  CROSS APPLY dates.firstOfYear(t.someDate, -1) f;
END;
