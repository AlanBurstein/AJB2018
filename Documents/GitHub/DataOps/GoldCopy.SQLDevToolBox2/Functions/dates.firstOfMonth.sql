...
--===== Autonomous use
 SELECT f.monthStart
 FROM dates.firstOfMonth(@date, @months) f;

--===== Use against a table
 SELECT st.someDateCol, f.monthStart
 FROM SomeTable st
 CROSS APPLY dates.firstOfMonth(st.someDateCol, 5) f;
...
--==== Basic use ->
  ...
  SELECT f.monthStart FROM dates.firstOfMonth(@date,  0) f; -- first of this month
  SELECT f.monthStart FROM dates.firstOfMonth(@date,  1) f; -- first of next month
  SELECT f.monthStart FROM dates.firstOfMonth(@date, -1) f; -- first of last month
...
--==== against a table ->
  ...
  FROM @table t
  CROSS APPLY dates.firstOfMonth(t.someDate, -10) f;
