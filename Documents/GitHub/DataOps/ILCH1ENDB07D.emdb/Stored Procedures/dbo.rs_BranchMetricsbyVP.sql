...
create Procedure [dbo].[rs_BranchMetricsbyVP]
...
and loanFolder not in ('(Archive)','(Trash)', 'Samples')
 ...
 and   pdlo.PaidLocode  in  	(select SPLITVALUES 
...
--------- Groups ---------------------------------------------------------------
 ...
 region,
  pdlo.PaidLocode as costcenterid,
 e.displayname  as costcenter, 
  e.displayname as costcentername,
...
0 as  'PurchaseLoanAmt',
  ...
  e.displayname as LoanOfficerName,
0 as LoanAmount,
0 as loanCount,
...
from
 ...
 emdb.dbo.PaidLo pdlo 
 left outer join  chilhqpsql05.Admin.corp.employee e on e.employeeid = pdlo.PaidLOCode
 left outer join  chilhqpsql05.Admin.corp.title t on t.titleid = e.titleid
 left outer join  chilhqpsql05.Admin.corp.costcenter cc on cc.costcenterid = e.costcenterid
 --left outer join chilhqpsql05.Admin.corp.costcenter c on c.costcenter = t.costcenter
  left outer join  chilhqpsql05.Admin.corp.titlecount tc on tc.costcenter = e.titleid
 left outer join chilhqpsql05.Admin.corp.region r on r.regionid = cc.regionid
