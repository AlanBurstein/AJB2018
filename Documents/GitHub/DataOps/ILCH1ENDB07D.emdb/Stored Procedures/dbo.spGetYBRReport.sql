...
CREATE procedure [dbo].[spGetYBRReport]
...
-- Allow Prequal addresses when the milestione is greater than 'started'
	AND (lf.IsPrequalifiedLoan = 0 OR (lf.IsPrequalifiedLoan = 1 AND dli.LoanMilestoneID not in( 16,21 ) )
	...
	-- mava changed the above code on 10.29.15 to the below
	AND fld.EstimatedClosingDate >= '2014-01-01'    -- est closing date
...
SELECT
    ...
    fld.CDSignedByAllDate, -- added by Mava on 10.2.15 for TRID
    dli.UseNewLeCdGfeHud, -- added by Mava on 10.2.15 for TRID
