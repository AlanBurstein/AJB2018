...
CREATE procedure [dbo].[spGetYBRReportCostCenter]
...
-- Allow Prequal addresses when the milestione is greater than 'started'
	...
	AND 
		(  ---- get employee(s)
			...
			OR (@GroupBy = 'ALL' AND (
										(eilc.EmployeeID IN (SELECT EmployeeID FROM #EmpInfoKeyTemp))
										OR (eimc.EmployeeID IN (SELECT EmployeeID FROM #EmpInfoKeyTemp))
										OR (eilo.EmployeeID IN (SELECT EmployeeID FROM #EmpInfoKeyTemp))
									)	
				)	
