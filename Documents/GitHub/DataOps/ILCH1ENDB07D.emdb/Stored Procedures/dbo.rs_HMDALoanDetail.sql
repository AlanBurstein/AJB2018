...
CREATE procedure [dbo].[rs_HMDALoanDetail]   --Brian's query 
@StartDate datetime,
@EndDate datetime
...
as
  ...
  _748,
	...
	case
		--when
		--	nullif(_4004, '') is null or _3174 = 'Y'
		--	or _1531 = 'No co-applicant' or _478 = 'No co-applicant'
		--then 5
	...
	case
		when _188 = 'Y' and isnull(_479, '') <> 'FaceToFace' then '6'
			...
			case when _1529 = 'Y' or _1530 = 'Y' then '6' else '' end
			--case _1529 when 'Y' then '6' else '' end +
			--case _1530 when 'Y' then '7' else '' end
		, ''), '6')
	end as [APRACE],
	case
		--when
		...
		when _189 = 'Y' and isnull(_479, '') <> 'FaceToFace' then '6'
			...
			case when _1537 = 'Y' or _1538 = 'Y' then '6' else '' end
			--case _1537 when 'Y' then '6' else '' end +
			--case _1538 when 'Y' then '7' else '' end
		, ''), '6')
	end as [CAPRACE],
	case
		when _188 = 'Y' and isnull(_479, '') <> 'FaceToFace' then '3'
...
--inner join emdb.emdbuser.LOANXDB_S_08 ls08 on x.XrefId = ls08.XrefId
--inner join emdb.emdbuser.LOANXDB_S_09 ls09 on x.XrefId = ls09.XrefId
...
--inner join emdb.emdbuser.LOANXDB_N_03 ln03 on x.XrefId = ln03.XrefId
--inner join emdb.emdbuser.LOANXDB_N_04 ln04 on x.XrefId = ln04.XrefId
--inner join emdb.emdbuser.LOANXDB_N_05 ln05 on x.XrefId = ln05.XrefId
--inner join emdb.emdbuser.LOANXDB_N_06 ln06 on x.XrefId = ln06.XrefId
--inner join emdb.emdbuser.LOANXDB_N_07 ln07 on x.XrefId = ln07.XrefId
--inner join emdb.emdbuser.LOANXDB_N_08 ln08 on x.XrefId = ln08.XrefId
--inner join emdb.emdbuser.LOANXDB_N_09 ln09 on x.XrefId = ln09.XrefId
inner join emdb.emdbuser.LOANXDB_D_01 ld01 on ls.XrefId = ld01.XrefId
--inner join emdb.emdbuser.LOANXDB_D_02 ld02 on x.XrefId = ld02.XrefId
--inner join emdb.emdbuser.LOANXDB_D_03 ld03 on x.XrefId = ld03.XrefId
--inner join emdb.emdbuser.LOANXDB_D_04 ld04 on x.XrefId = ld04.XrefId
