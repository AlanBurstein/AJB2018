...
CREATE Procedure [dbo].[rs_YBR_Branch_exceptions_testuserlogin]
...
--   exec dbo.[rs_YBR_Branch_exceptions_testuserlogin] 'dgorman', 'LC', 'Laura Manrique' ,'all'
--   exec dbo.[rs_YBR_Branch_exceptions_testuserlogin] 'dgorman', 'MC', 'Meaghan Bobola' ,'all'
--'mvacirca'
--2001-01-01 00:00:00.000
--	'UWapprvNotSubmit'  --'all'

--   exec dbo.rs_YBR_Branch_exceptions 'HPatel', 'VP', '5914' ,'all'
...
-------- ==========================================================================================
  ...
  distinct
    ...
    ls.loanofficerid,
    sa.userlogin as sa,
    empmc.userlogin as mc,
    emplc.userlogin as lc,
...
from
	...
	left outer join chilhqpsql05.admin.corp.employee sa  on (sa.employeeid = e.salesassistiantid or 
...
and 
	...
	and     -- get userid (logon)johne
		( 
		@UserName in (e.Userlogin, empLC.userlogin, empmc.userlogin, sa.userlogin) --VP, LC, MC
		  or
...
--Was this: @UserName in (select SPLITVALUES from dbo.DelimitedListToVarcharTableVariable(dbo.allmanagers(dem.empid), ',')) --MGR
		...
		'cstackhouse', 'ahardas', 'cmorgan', 'jpike') 
		)
