...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[rs_YBR_NY]
...
--   exec dbo.rs_YBR_Branch_exceptions_testdg 'dgorman', 'VP', 2197 ,'all'
--2001-01-01 00:00:00.000
--	'UWapprvNotSubmit'  --'all'

--   exec dbo.rs_YBR_NY 'kguy', 'MC', 'kguy' ,'all'
...
CASE      --find brokered loans first
	 ...
	 WHEN ((_2303 > '2000-01-01' and 2301 is null 
...
END  AS PrevMilestoneGroup,
	...
	--END as Investor,
	ls09.lockrate_2278 as ls09inv,
	inv.lockrate_2278 as viewinv,
	CASE when  --isnull(ls09.lockrate_2278,'') <> '' and  
			ls09.lockrate_2278 <> inv.lockrate_2278   
...
--LC rule -  CTC  > 2 Days after Conditions submitted to UW
 ...
 --             ((_2305 < (dbo.DateAddWeekday(6,( Case when _CX_CLSSCHED_1 > _763 then _CX_CLSSCHED_1 else _763 End))) AND _19  = 'Purchase') ))
  ...
  --MC rule - Application IN < 3 weekdays  of Application Out -- 		
    ...
    --NY fields 
   _CX_CX_NY_CONDO_COOP_APPROVED,
   -- Commented Out cx_aztecs_out on 1/29/2015, rcorro
	--_CX_AZTECS_OUT,
	--'1/1/12' as _CX_NY_STOCK_LEASE_ORDERED,
	 _CX_NY_STOCK_LEASE_ORDERED,
	 _CX_NY_STOCK_LEASE_RECIEVED,
	_CX_NY_CEMA_ORDERED,
	--'1/1/12' as  _CX_NY_CEMA_IN,
	  _CX_NY_CEMA_IN,
	 _CX_CONDODOCS_10,
	 _CX_CONDORECV_10,
	 _CX_INVAPPROVAL,
	 _CX_LOANCOMMITSENT_1,
	 _CX_NY_BOARD_IN,
	--'1/1/12' as _CX_CX_NY_BOARD_APPROVED,
	_CX_CX_NY_BOARD_APPROVED,
	--'1/1/12' as _CX_COOPBOARD_APPR_EXP,
	--_CX_COOPBOARD_APPR_EXP Commented out on 1/16/2015 by rcorro
	 --_CX_COOPBOARD_APPR_EXP,
	 _CX_BROKER_UW,
	 _CX_BRKCTC,
	 _CX_BRKAPPROVE_12,
	 _CX_BROKERCLOSE_5
...
and 
		 ((_2626 = 'Brokered' and _CX_BROKERCLOSE_5 is null) or _CX_FUNDDATE_1 is null ) 
...
--Was this: @UserName in (select SPLITVALUES from dbo.DelimitedListToVarcharTableVariable(dbo.allmanagers(dem.empid), ',')) --MGR
		...
		'kwoodruff', 'frankc', 'johne', 'mknopf' , 'bconn', 'jweaver','robs','mdye', 'nathanasiou', 'rjones', 
		...
		'cstackhouse', 'ahardas', 'cmorgan', 'jpike', 'vhenderson', 'ltitiyevsky' , 'rhacek', 'sstephen',			 
		'ghafner', 'mward', 'KShattuck', 'mharmon', 'sives', 'tsculco', 'lwalker', 'lbrandow', 'mharmon', 'smozes', 'sbetesh', 'sbarcomb', 'rcorro' ) 
		) 
