...
--   exec reports.rs_BranchMetricsbybranchandvp  '2012-06-01', '2012-06-02', 1001

CREATE Procedure [reports].[rs_BranchMetricsbyBranchandVP]
...
case when  ( _1172 = 'FHA' or  _1172 = 'VA')  then 1 else 0
end as FhaVaLoanAmt,
...
case when  _19 = 'Purchase' then 1  else 0
end as PurchaseLoanAmt,

ls.LoanOfficerName,

case when  lockrate_2965 is null then _2
...
case when AppOrderToRecvd < appSentToAssgnToUW 
	 then appSentToAssgnToUW - AppOrderToRecvd 
	 else appSentToAssgnToUW 
end  as    'AppOuttosentUW',

case when   _19 = 'Purchase' then 
...
UWDecsionToAssnToClose  as    'UW Decision Expected to Assigned to Close',
...
Case 
 ...
 ---- Section 5  ---------------------- Quality  ------------------------------------
 1  as 'UWdecisionLoanCount',
 
 
 
 CAST(_CX_UWC_COUNT_MC AS Int)  as 'CountofRoleMortgageConsultant',
			 
 _CX_NUMUWCONDITION   as '#MC Conditions',				----1

case   WHEN _CX_NUMUWCONDITION < 1 then 0 	else 1
END as 'NumOfLoansMC_COND',

CASE WHEN  _CX_PROBLOAN_5  = 'Y' then 1 
	else 0 
...
CASE WHEN _CX_PROBLOANCLS_14 = 'Y' 	then 1 
	else 0 
...
--CASE when ls01._984 IN ( uw._984)  then 'll' else 
	...
	case when _CX_UWRUSH_1 = 'Yes' then 1 
...
0 as [LoanAmount_mip_ff],
 ...
 ---- Section 4  ---------------------- Efficiency Dates ----------------------------
  ...
  null as    'Purch_AppOuttoUWsubmit',
  null as    'Refi_AppOuttoUWsubmit', 
 null as 'Date Funded',
