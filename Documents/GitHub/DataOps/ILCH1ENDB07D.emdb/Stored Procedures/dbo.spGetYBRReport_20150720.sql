...
CREATE procedure [dbo].[spGetYBRReport_20150720]
...
--IF (@GroupBy = 'VP')  -- REMOVED  BY MAVA
--BEGIN  -- REMOVED  BY MAVA
...
FROM LoanWarehouse.dbo.factLoan lf
	...
	--LEFT OUTER JOIN chilhqpsql05.admin.corp.employeeandallmanagers2 dem  on dem.empid =  eilo.employeeId -- MAVA REMOVED
...
--	AND (ligi.StreetAddress IS NOT NULL OR (lf.IsPrequalifiedLoan = 1 AND lm.LoanMilestoneID = 16  AND ( isnull( ligi.StreetAddress, '' ) = '') )
	...
	--and @EmployeeInfoKey Select #EmpInfoKeyTemp.Employeeinfokey FROM #EmpInfoKeyTemp)	
