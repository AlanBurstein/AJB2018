...
create procedure [reports].[spGetTitleCompanyReportSummaryByCostCenter]
...
----------------------------------------------------------------------------------------------------------------------------------
	who	when		what
----------------------------------------------------------------------------------------------------------------------------------
	sethm	04/19/13	initial rev of proc
	sethm	07/17/13	added ability to select multiple states for a parameter
...
begin
	...
	if @UserName not in ( 'smueller', 'nejohnson', 'hgordon', 'dgorman', 'nathanasiou', 'johne', 'bcarrara', 'dmoran', 'jroda' )
	begin
		return 0
	end
	...
	if @LoanOfficerID <> 0
	begin
		insert	#EmployeeInfo ( EmployeeID, CostCenterID )
		select	@LoanOfficerID, CostCenterID
	...
	begin
		insert	#EmployeeInfo ( EmployeeID, CostCenterID )
		select	E.EmployeeID, E.CostCenterID
	...
	select	C.CostCenter, C.CostCenterName, LoanVolume, LoanUnits, ( LoanUnits + 0.0 ) / ( @TotalLoanUnits + 0.0 ) as PercentageOfTotal,
	...
	from	#TitleCompanyInfo T
		inner join admin.corp.CostCenter C on T.CostCenterID = C.CostCenterID
	order	by 3 desc, 1
