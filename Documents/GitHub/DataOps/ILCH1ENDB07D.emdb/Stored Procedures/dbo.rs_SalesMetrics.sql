...
SET ANSI_NULLS ON
GO
--             exec rs_SalesMetrics '2012-05-01', '2012-05-31'
CREATE Procedure [dbo].[rs_SalesMetrics]
...
--------- Groups ---------------------------------------------------------------
  ...
  Case 
    ...
    --- added as sales metrics
    e.startDate,
	
	Case when e.startDate <= DATEADD(mm,-1,getdate()) 
	and e.endDate is null 
	then 'active' else 'inactive' end as  'priormonthVP',
	e.seniorityDate,
	e.enddate,
	e.terminationnotes ,
  ---- end added as sales metrics
...
--DATEDIFF(d,origlockdate, Log_MS_Date_Processing ) as    'Lock to Processing',
...
Case 
 ...
 --DATEDIFF(d, origlockdate , Log_MS_Date_AssignedtoUW)    as    'Lock to Assigned to UW',
Case 
...
--  DATEDIFF(d,origlockdate , Log_MS_Date_Approval ) as 'Lock to Approval',
Case 
...
--  DATEDIFF(d,origlockdate, Log_MS_Date_DocsSigning ) as 'Lock to Doc Signing',
Case 
...
--    DATEDIFF(d, origlockdate   , _CX_FUNDDATE_1 ) as 'Lock to Fund',
...
and d.division  in ('Retail', 'Direct', 'Online')
and (cc.costcenter > 1 and cc.costcenter < 4000)  
  --not in ('Admin/Operations') and d.division is not null
...
from
 ...
 left outer join  chilhqsqldb.Warehouse.operations.CostPerLoan cpl on cpl.costcenter = cc.costcenter and
	...
	where d.division  in ('Retail', 'Direct', 'Online')	
	and (cc.costcenter > 1 and cc.costcenter < 4000)  
--   exec rs_SalesMetrics_test  '2012-05-01', '2012-05-10'
UNION
...
null as Expense,
null as cpl_units,
case when facebook = 'Yes' then 1 else 0 end as Facebook,
case when twitter = 'Yes' then  1 else 0 end as Twitter,
case when linkedin = 'Yes' then 1 else 0 end as linkedin,
case when [Current Postcard] = 'Yes' then 1 else 0 end as 'cpostcard',
case when [Weekly Newsletter] = 'Yes' then 1 else 0 end as 'wknews',
case when [Monthly Consumer Newsletter] = 'Yes' then 1 else 0 end as 'moconsnews',
case when [Monthly Realtor Newsletter] = 'Yes' then 1 else 0 end as 'morealnews',
mc.contacts as contacts
 
from
 chilhqsqldb.admin.corp.employee e
 left outer join chilhqsqldb.admin.corp.costcenter cc on cc.costcenterid = e.costcenterid
 left outer join chilhqsqldb.warehouse.mktg.EmployeeSocialNetworkStatus sns  on sns.employeeid = e.employeeid
 left outer join chilhqsqldb.warehouse.mktg.VPCampaignsignup mc on mc.employeeid = e.employeeid
