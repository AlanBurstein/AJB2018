...
SET ANSI_NULLS ON
GO
CREATE Procedure [reports].[rs_BasicData_adalparams1_SM]
  @AllSales     varchar(20),
...
from
	...
	left outer join admin.corp.employee e on e.locode = ls03._CX_FINALOCODE_4
	left outer join admin.corp.office o on o.officeid = e.officeid
	left outer join admin.corp.costcenter cc on cc.CostCenterID = e.costcenterid
...
where 
	...
	and
		 isnull( d.division, '' ) like (case @AllSales
			when 'Retail' then '%Retail%'
			when 'Direct' then '%Direct%'
			when 'online' then '%Online%'
			when 'all' then isnull( d.division, '' )
			 end)
...
SELECT
	...
	0 as TOTemp
from
	admin.reports.SMC_ClosedLoans smc
	left outer join admin.corp.Employee e on e.employeeid = smc._CX_paidlocode_3
	left outer join admin.corp.office o on o.officeid = e.officeid
	left outer join admin.corp.costcenter cc on cc.CostCenterID = e.costcenterid
	left outer join admin.corp.Region r ON r.regionID = cc.RegionID 
	left outer join admin.corp.division d on d.DivisionID = r.divisionid
