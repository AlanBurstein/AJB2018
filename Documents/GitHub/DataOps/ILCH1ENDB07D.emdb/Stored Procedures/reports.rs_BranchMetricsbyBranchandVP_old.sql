...
GO
    ---------         
---Original query used before it was limited to just funded date range. D Gorman 9/21/12
		
--   exec reports.rs_BranchMetricsbybranch  '2012-06-01', '2012-06-02', 1001

create Procedure [reports].[rs_BranchMetricsbyBranchandVP_old]
...
case when _cx_funddate_1 between @StartDate and @EndDate then ln02._2 else 0
			end as LoanAmount,
		
case when _cx_funddate_1 between @StartDate and @EndDate then 1 else 0 
			end as LoanCount,
...
case when _cx_funddate_1 between @StartDate and @EndDate and 
	 ( _1172 = 'FHA' or  _1172 = 'VA')  then 1 else 0
end as FhaVaLoanAmt,

ls02._364 as LoanNumber,
...
case when _cx_funddate_1 between @StartDate and @EndDate 
				and  _19 = 'Purchase' then 1  else 0
end as PurchaseLoanAmt,

ls.LoanOfficerName,

case when _cx_funddate_1 between @StartDate and @EndDate  and lockrate_2965 is null then _2
	 when _cx_funddate_1 between @StartDate and @EndDate  and lockrate_2965 = 0 then _2
  ...
  -- 2298	LOANXDB_D_01 Sent to UW

case when _cx_funddate_1 between @StartDate and @EndDate  then
(case
	when DATEDIFF(d,_CX_APPSENT_1, _2298 )  < -30 then -10
	when DATEDIFF(d,_CX_APPSENT_1, _2298 )  > 30 then 30
	else DATEDIFF(d,_CX_APPSENT_1, _2298) end ) else 0
end   as    'AppOuttosentUW',
...
(Case 
	...
	when DATEDIFF(d,origlockdate, Log_MS_Date_Processing)   > 60 then 60
	else DATEDIFF(d,origlockdate, Log_MS_Date_Processing) end) else 0 
end   as   'Lock to Processing',
--DATEDIFF(d,emdbuser, Log_MS_Date_Processing ) as    'Lock to Processing',
...
(Case 
	...
	when DATEDIFF(d,_CX_APPSENT_1, _CX_APPRCVD_1 )  > 60 then 60
	else DATEDIFF(d,_CX_APPSENT_1, _CX_APPRCVD_1 ) end) else 0
...
(Case 
	...
	when DATEDIFF(d, ld02._cX_APPRRECTRANS_10 , Log_MS_Date_AssignedtoUW)  > 60 then 60
	else DATEDIFF(d, ld02._cX_APPRRECTRANS_10 , Log_MS_Date_AssignedtoUW) end) else 0
end   as    'Appraisal In to Assigned to UW',

 --DATEDIFF(d, emdbuser , Log_MS_Date_AssignedtoUW)    as    'Lock to Assigned to UW',
case when _cx_funddate_1 between @StartDate and @EndDate  then
(Case 
	when DATEDIFF(d, origlockdate , Log_MS_Date_AssignedtoUW) < -60 then -60
	when DATEDIFF(d, origlockdate , Log_MS_Date_AssignedtoUW)  > 60 then 60
	else DATEDIFF(d, origlockdate , Log_MS_Date_AssignedtoUW) end) else 0
...
(Case 
	...
	when DATEDIFF(d, Log_MS_Date_UWDecisionExpected, Log_MS_Date_AssigntoClose)  > 60 then 60
	else DATEDIFF(d, Log_MS_Date_UWDecisionExpected, Log_MS_Date_AssigntoClose) end) else 0
end  as    'UW Decision Expected to Assigned to Close',

--  DATEDIFF(d,emdbuser , Log_MS_Date_Approval ) as 'Lock to Approval',
case when _cx_funddate_1 between @StartDate and @EndDate  then
(Case 
	when DATEDIFF(d,origlockdate , Log_MS_Date_Approval ) < -60 then -60
	when DATEDIFF(d,origlockdate , Log_MS_Date_Approval )  > 60 then 60
	else DATEDIFF(d,origlockdate , Log_MS_Date_Approval )end) else 0
end  as 'Lock to Approval',

--  DATEDIFF(d,emdbuser, Log_MS_Date_DocsSigning ) as 'Lock to Doc Signing',
case when _cx_funddate_1 between @StartDate and @EndDate  then
(Case 
	when DATEDIFF(d,origlockdate, Log_MS_Date_DocsSigning ) < -60 then -60
	when DATEDIFF(d,origlockdate, Log_MS_Date_DocsSigning )  > 60 then 60
	else DATEDIFF(d,origlockdate, Log_MS_Date_DocsSigning )end) else 0
end  as 'Lock to Doc Signing',


--    DATEDIFF(d, emdbuser   , _CX_FUNDDATE_1 ) as 'Lock to Fund',
case when _cx_funddate_1 between @StartDate and @EndDate  then
(Case 
	when DATEDIFF(d, origlockdate   , _CX_FUNDDATE_1 ) < -60 then -60
	when DATEDIFF(d, origlockdate   , _CX_FUNDDATE_1 )  > 60 then 60
	else DATEDIFF(d, origlockdate   , _CX_FUNDDATE_1 )end) else 0
end  as 'Lock to Fund',


 ---- Section 5  ---------------------- Quality  ------------------------------------
  case when _cx_funddate_1 between @StartDate and @EndDate  then 1 else 0
		end as 'UWdecisionLoanCount',
...
case when _cx_funddate_1 between @StartDate and @EndDate  then  
		 _CX_NUMUWCONDITION else 0 end  as '#MC Conditions',				----1
...
CASE
	when  _cx_funddate_1 between @StartDate and @EndDate  
...
CASE
	when   _cx_funddate_1 between @StartDate and @EndDate  
    then  _CX_UWNUMCNDOREVD end as 'LC: # Avg submission for approval',     -----3
...
--CASE when ls01._984 IN ( uw._984)  then 'll' else 'n/a' end as checkuw,
--CASE when ls01._984 IN ( uw._984)  then 'll' else 
	...
	--			 then _CX_UWC_COUNT_MC end as 'CountofRoleMortgageConsultant',

 CASE 
	WHEN  _cx_funddate_1 between @StartDate and @EndDate 
				 then (case when _CX_UWRUSH_1 = 'Yes' then 1 
 
	  when _CX_UWRUSH_1 = 'No' then 0 	
	  else _CX_UWRUSH_1 end)
	  else 0
 end as  'rushtoUW',	
------------------------------------------------------------------------------------
 ------------------------------------------------------------------------------------
...
--inner join emdb.emdbuser.LOANXDB_S_07 ls07 on ls.XrefId = ls07.Xref
...
inner join emdb.dbo.lockdate ld on ld.xrefid = ls.XRefID
...
where 
	_2 > 1  
    and
	--(_2301 between @StartDate and @EndDate  --@StartDate and @EndDate --Underwriting Approval Date
    ...
    and  ls01._984 = uw._984    -- no external underwriters
    
	and  lockrate_2278 not in  (   --- not brokered
...
0 as [LoanAmount_mip_ff],
 ...
 ---- Section 5  ---------------------- Quality  ------------------------------------
 0 as 'UWdecisionLoanCount',
 0 as 'CountofRoleMortgageConsultant',
...
--null as  'checkuw',
--null as 'CountofRoleMortgageConsultant',
0 as  'rushtoUW',
------------------------------------------------------------------------------------
 --COUNTS
...
--exec rs_BranchMetrics  '2012-01-01', '2012-04-30'
