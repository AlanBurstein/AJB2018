...
create procedure [reports].[spGetTitleCompanyReportByState]
...
begin
	...
	declare @TotalTitleTurnaroundLoans int
	...
	-- and return results
	select	_14 as StateName,
		isnull( T.TitleCompanyName, '(blank)' ) as TitleCompanyName,
	...
	group	by _14, isnull( T.TitleCompanyName, '(blank)' )

	-- get the total title turnaround times and loan units
	select	@TotalLoanUnits = sum( LoanUnits ), 
		@TotalTitleTurnaroundTime = sum( TitleTurnaroundTime ),
		@TotalTitleTurnaroundLoans = sum( TitleTurnaroundLoans )
	from	#StateInfo
	
	-- and return the results
	select	StateName, TitleCompanyName, LoanVolume, LoanUnits, ( LoanUnits + 0.0 ) / ( @TotalLoanUnits + 0.0 ) as PercentageOfTotal,
