...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[rs_YBR_usingEncompassEmpListMultiCostCenter]
...
-------- Description:	YBR revamp.  User passes in a comma delimited list of CostCenter numbers
--------				and what is returned is the data for all the cost centers
-------- ==========================================================================================
-------- Modifications:
...
-------- ==========================================================================================
   --test = Case when (_CX_APPSENT_1 > '2001-01-01' and _CX_APPRCVD_1 IS null) then 'YES' else 'No' end,
	...
	--END as Investor,
	CASE 
		WHEN isnull(ls09.lockrate_2278,'') <> '' and  ls09.lockrate_2278 <> inv.lockrate_2278   
...
----SC/VP/MC rule - Application OUT > Same Day of Send to Processing ( 1 day after)		
	...
	--END AS count_AppOut,
		WHEN ((dbo.DateAddWeekDay(1,Log_MS_Date_Processing)) < getdate() and _CX_APPSENT_1 is null) 
			OR (_CX_APPSENT_1 > (dbo.DateAddWeekDay(1,Log_MS_Date_Processing))) then 1 
		ELSE 0
	 END AS count_AppOut, 
...
--MC rule -  appraisal ordered > same day of send to processing	
	...
	CASE 
		WHEN (_CX_APPRAISALREQ <> 'No' and _1172 <> 'FHA' and (Log_MS_Date_Processing > '2001-01-01' and _CX_APPRORDERTRANS_10 is null))
			...
			OR 
			( _CX_APPRAISALREQ <> 'No' and _1172 = 'FHA' and (_CX_APPRCVD_1 > '2001-01-01' and _CX_APPRORDERTRANS_10 is null))
			OR
...
--MC rule -  UWSubmission for REFI  > 1 day of appraisal in
	...
	CASE	
		WHEN _19 like '%Refi%' and(( ((dbo.DateAddWeekDay(2,_CX_APPRRECTRANS_10)) < GETDATE()  and _2298 IS NULL)) 
	...
	CASE 
		WHEN _2626 = 'Brokered' or (isnull(ls09.lockrate_2278,'') <> '' and ls09.lockrate_2278 <> inv.lockrate_2278) then _CX_BRKCTC 
		ELSE _2305 
	END AS UWCTC,
...
--LC rule -  CTC  > 2 Days after Conditions submitted to UW
	...
	CASE	
		WHEN _19 = 'Purchase' 
			...
			OR (_19 like '%Refi%' and _1811 IN ('SecondHome', 'Investor') and (_2305 >=  (dbo.DateAddWeekDay(6,_762))))
 ...
 --             ((_2305 < (dbo.DateAddWeekday(6,( Case when _CX_CLSSCHED_1 > _763 then _CX_CLSSCHED_1 else _763 End))) AND _19  = 'Purchase') ))
	...
	CASE	
		...
		( _cx_apprreviewed > (dbo.DateAddWeekDay(1,_2301))) then 1 
		ELSE 0
	END AS count_aprvlREVIEWED,
	CASE	
		WHEN((dbo.DateAddWeekDay(1,_2301)) < GETDATE() and  _cx_apprreviewed IS NULL) 
		OR ( _cx_apprreviewed > (dbo.DateAddWeekDay(1,_2301))) 
		OR _cx_apprreviewed is null THEN 0 
		ELSE 1
