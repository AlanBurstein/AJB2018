...
create Procedure [dbo].[rs_SalesMetrics_copy notmessedup]
...
Case 
	...
	when DATEDIFF(d,_761, Log_MS_Date_Processing)   > 60 then 60
	else DATEDIFF(d,_761, Log_MS_Date_Processing)  
...
Case 
	...
	when DATEDIFF(d, _761 , Log_MS_Date_AssignedtoUW)  > 60 then 60
	else DATEDIFF(d, _761 , Log_MS_Date_AssignedtoUW) 
...
Case 
	...
	when DATEDIFF(d,_761 , Log_MS_Date_Approval )  > 60 then 60
	else DATEDIFF(d,_761 , Log_MS_Date_Approval )
...
Case 
	...
	when DATEDIFF(d,_761, Log_MS_Date_DocsSigning )  > 60 then 60
	else DATEDIFF(d,_761, Log_MS_Date_DocsSigning )
...
Case 
	...
	when DATEDIFF(d, _761   , _CX_FUNDDATE_1 )  > 60 then 60
	else DATEDIFF(d, _761   , _CX_FUNDDATE_1 )
end  as 'Lock to Fund',
...
null as Expense,
null as cpl_units,
case when facebook = 'Yes' then 1 end as Facebook,
case when twitter = 'Yes' then  1  end as Twitter,
case when linkedin = 'Yes' then 1 end as linkedin
