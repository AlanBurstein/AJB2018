...
SET ANSI_NULLS ON
GO
  --- exec [reports].[rs_BranchMetricsbyBranch] '2013-01-01', '2013-01-31', null, '2012-11-01'


CREATE Procedure [reports].[rs_BranchMetricsbyBranch]
...
Case
when _748 between @StartDate and @EndDate 
and (_1172 = 'FHA' or  _1172 = 'VA')  then 1
end as FhaVaLoanAmt,
...
CASE when _748 between @StartDate and @EndDate and 
_2 < 1 then 0 else 1 end as LoanCount,
...
---
--left outer join chilhqsqldb.Admin.corp.Titlecount title on title.costcenter = cc.costcenter and title.Activeperiod = @StartDate
...
_2 > 1  
	and		(_2301 between @StartDate and @EndDate  --@StartDate and @EndDate --Underwriting Approval Date
			...
			or _748 between @StartDate and @EndDate)  --closed date
...
from
	chilhqsqldb.Admin.corp.costcenter cc 
 left outer join chilhqsqldb.Admin.corp.titlecount title on title.costcenter = cc.costcenter 
	and title.Empactiveperiod = @StartDate 
...
--select * from chilhqsqldb.Warehouse.operations.CostPerLoan
GO
