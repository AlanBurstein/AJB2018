...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[rs_YBR_usingEncompassEmpList]
...
-------- rcorro				02/17/2015			Added mwalsh to the all access list
-------- ==========================================================================================
...
--MC rule -  UWSubmission for REFI  > 1 day of appraisal in
 ...
 or _2298 is null
  then 0 else 1
	...
	case when _2626 = 'Brokered' or (isnull(ls09.lockrate_2278,'') <> '' and ls09.lockrate_2278 <> inv.lockrate_2278) then _CX_BRKCTC else _2305 end AS UWCTC,
...
--LC rule -  CTC  > 2 Days after Conditions submitted to UW
 ...
 --             ((_2305 < (dbo.DateAddWeekday(6,( Case when _CX_CLSSCHED_1 > _763 then _CX_CLSSCHED_1 else _763 End))) AND _19  = 'Purchase') ))
	...
	case when _2626 = 'Brokered' or (isnull(ls09.lockrate_2278,'') <> '' and ls09.lockrate_2278 <> inv.lockrate_2278) then _CX_CND_SENT_BROKER else
	coalesce(ld04._CX_CONDITIONSUBMIT_5,ld04._CX_CONDITIONSUBMIT_4,ld04._CX_CONDITIONSUBMIT_3,ld04._CX_CONDITIONSUBMIT_2,ld01._CX_CONDITIONSUBMIT_1 ) end as CondSubmit,
...
--Was this: @UserName in (select SPLITVALUES from dbo.DelimitedListToVarcharTableVariable(dbo.allmanagers(dem.empid), ',')) --MGR
			...
			,'ltrout', 'ntaylor','mwalsh', 'lschreiber'
			) 
		)
GO
