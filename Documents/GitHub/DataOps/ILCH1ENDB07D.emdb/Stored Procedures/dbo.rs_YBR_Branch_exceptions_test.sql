...
CREATE Procedure [dbo].[rs_YBR_Branch_exceptions_test]
...
--   exec dbo.rs_YBR_Branch_exceptions_test 'lwalker', 'LC', 'Pamela Camiscioni' ,'all'
...
CASE      --find brokered loans first
	...
	WHEN Log_MS_Lastcompleted = 'Doc Signing' 
			and ( _2626 <> 'Brokered' or (isnull(ls09.lockrate_2278,'') <> '' 
			and ls09.lockrate_2278 = inv.lockrate_2278)) 
	then 10
...
from
	...
	--left outer join chilhqpsql05.admin.corp.EmployeeAndAllManagers2 dem  on dem.empid =  empMC.employeeId
	--left outer join chilhqpsql05.admin.corp.EmployeeAndAllManagers2 dem  on dem.empid =  emcLC.employeeId
		--left outer join chilhqpsql05.admin.corp.EmployeeAndAllManagers2 dem  on dem.empid =  e.employeeId 
...
--Was this: @UserName in (select SPLITVALUES from dbo.DelimitedListToVarcharTableVariable(dbo.allmanagers(dem.empid), ',')) --MGR
		...
		'ghafner', 'mward', 'KShattuck', 'mharmon', 'sives', 'tsculco', 'lwalker', 'lbrandow', 'mharmon' ) 
		) 
