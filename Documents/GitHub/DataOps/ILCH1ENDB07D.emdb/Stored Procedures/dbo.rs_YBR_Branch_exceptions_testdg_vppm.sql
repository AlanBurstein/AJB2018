...
CREATE Procedure [dbo].[rs_YBR_Branch_exceptions_testdg_vppm]
...
--       exec dbo.rs_YBR_Branch_exceptions_testdg_VPPM 'dgorman', 'VP', 6088 ,'all'
...
----exec dbo.rs_YBR_Branch_exceptions_testdg 'mortiz', 'LC', 'Julissa Eusebio' ,'all'


----exec dbo.rs_YBR_Branch_exceptions_testdg_VPPM 'mortiz', 'MC', 'Suzanne Dzolan' ,'all'
----exec dbo.rs_YBR_Branch_exceptions_testdg 'mortiz', 'MC', 'Suzanne Dzolan' ,'all'
...
and 
	...
	and 
		...
		(rtrim(ltrim(_317))in 
				(select SPLITVALUES 
			from dbo.DelimitedListToVarcharTableVariable(isnull(@Employee,_317), ','))))	
...
--Was this: @UserName in (select SPLITVALUES from dbo.DelimitedListToVarcharTableVariable(dbo.allmanagers(dem.empid), ',')) --MGR
		...
		'ghafner', 'mward', 'KShattuck', 'ylopez', 'bhawley', 'jpugh', 'nejohnson') 
		)
