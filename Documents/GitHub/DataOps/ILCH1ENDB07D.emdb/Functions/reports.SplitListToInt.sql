...
-- Description:	Splits a delimited list to a table of ints
-- =============================================
...
CREATE FUNCTION [reports].[SplitListToInt] 
(	
...
(
	...
	t(Value) as (select substring(@list, l.n, l.nl) from cteLen l)
	-- Just the elements that can be converted to int
	select convert(int, Value) as Value from t where isnumeric(t.Value)=1
)
