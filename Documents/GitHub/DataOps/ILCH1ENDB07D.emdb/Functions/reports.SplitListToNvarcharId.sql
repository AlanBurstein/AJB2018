...
-- Description:	Splits a delimited list to a table of nvarchars with an id
-- =============================================
...
CREATE FUNCTION [reports].[SplitListToNvarcharId] 
(	
	@list nvarchar(4000), @delimiter nvarchar(10)
)
...
(
	...
	cteTally(n) as (
		...
		select top (isnull(datalength(@list)/2,0)) row_number() over (order by (select null)) from e4
	),
	cteStart(n) as (
		...
		select t.n + datalength(@delimiter)/2 from cteTally t
		where @delimiter <> N'' and
			substring(@list, t.n, datalength(@delimiter)/2) = @delimiter
	),
	cteLen(n,nl) as (
	...
	select
		Id = row_number() over (order by l.n),
		Value = substring(@list, l.n, l.nl)
	from cteLen l
)
