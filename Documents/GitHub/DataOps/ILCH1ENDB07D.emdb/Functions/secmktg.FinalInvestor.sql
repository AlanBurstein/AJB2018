...
-- Description:	Simplify the investor name
-- =============================================
CREATE FUNCTION [secmktg].[FinalInvestor] 
(
	@Investor varchar(64)
)
...
RETURN (
	select Investor = case 
		...
		when @Investor in (
			'Bank of America Correspondent','BOA','Countrywide','Bank of America','b of a',
			'Countrywide Correspondent','Countrywide Securities Corporation'
		) then 'BofA'
		...
		when @Investor in (
			'CHASE RURAL','Chase Rural Development - Corresponde','Chase Rural Development - Correspondent'
		) then 'Chase RD'
		...
		when @Investor in (
			'National Broker','National City','National City Broker','National City Correspondent',
			'NATIONAL CITY HOME EQUITY','National City Mortgage Broker','National City Mortgage Correspondent'
		) then 'National City'
		...
		when @Investor in (
			'Taylor Bean','Taylor Bean & Whitaker','TAYLOR BEAN AND WHITAKER','TAYLOR BEAN WHITAKER','Taylor Bean Whitaker Correspondent'
		) then 'Taylor Bean & Whitaker'
		when @Investor in ('TCF','TCF BANK','TCF Bank Broker') then 'TCF Bank'
		when @Investor in (
			'U.S. Bank Home Mortgage','U.S. BANK','US Bank','US Bank  EZD','US Bank - EZD','US Bank Broker',
			'US Bank Correspondent','US Bank Correspondent - EZD','US Bank EZD','US Bank -EZD','USBANK'
		) then 'US Bank'
		when @Investor like 'USBANK%' then 'US Bank'
		when @Investor in ('Wachovia Broker') then 'Wachovia'
		when @Investor in (
			'WAMU Broker','WAMU Correspondent','Washington Mutual','Washington Mutual Bank WHS','Washington Mutual Broker'
		) then 'Washington Mutual'
