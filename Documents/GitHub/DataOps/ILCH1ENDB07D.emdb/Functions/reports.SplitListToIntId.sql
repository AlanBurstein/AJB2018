...
-- Description:	Splits a delimited list to a table of ints with an id
-- =============================================
...
CREATE FUNCTION [reports].[SplitListToIntId] 
(	
...
(
	...
	t(Id,Value) as (
		select row_number() over (order by l.n), substring(@list, l.n, l.nl) from cteLen l
	)
	-- Just the elements that can be converted to int
	select Id, convert(int, Value) as Value from t where isnumeric(t.Value)=1
)
