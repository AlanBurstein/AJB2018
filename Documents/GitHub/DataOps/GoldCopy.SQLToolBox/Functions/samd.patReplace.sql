...
--===== Basic Syntax Example
 SELECT pr.NewString
 FROM samd.patReplace(@String,@Pattern,@Replace);

--===== Replace numeric characters with a "*"
 SELECT pr.NewString
 FROM samd.patReplace('My phone number is 555-2211','[0-9]','*') pr;
...
--==== Using againsts a table
 ...
 FROM @table t
 CROSS APPLY samd.patReplace(t.oldstring,'[0-9]','*') pr;
...
SELECT NewString = 
  CASE WHEN @pattern+@replace IS NULL THEN NULL
       WHEN @string = '' THEN '' ELSE ((
    SELECT
      CASE 
      ...
      END
    FROM iTally
    FOR XML PATH(''), TYPE).value('text()[1]','varchar(max)')) END;
GO
