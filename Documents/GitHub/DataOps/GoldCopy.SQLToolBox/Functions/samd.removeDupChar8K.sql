...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [samd].[removeDupChar8K](@string varchar(8000), @char char(1))
...
Syntax:
 ...
 CROSS APPLY samd.removeDupChar8K(@string) rd;
...
--===== #1: Remove Duplicate Spaces
 ...
 SELECT NewString FROM samd.removeDupChar8K(@string, char(32));
...
--===== #2: Against a table
 ...
 BEGIN
   ...
   FROM @sometable t
   CROSS APPLY samd.removeDupChar8K(t.someText, '!') rd;
 END
----------------------------------------------------------------------------------------
Revision History:
