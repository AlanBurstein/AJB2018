...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [samd].[removeDupChar](@string varchar(max), @char char(1))
...
Syntax:
 ...
 CROSS APPLY samd.removeDupCharMax(@string) rd;

Parameters:
 @string - varchar(max); the input string to clean duplicate spaces from
...
--===== #1: Remove Duplicate Spaces
 DECLARE @string varchar(max) = '1 space 2 space  whitespace    blue space     !'
 SELECT NewString FROM samd.removeDupChar(@string, char(32));
...
--===== #2: Against a table
 ...
 BEGIN
  ...
  FROM @sometable t
  CROSS APPLY samd.removeDupChar(t.someText, '!') rd;
 END
----------------------------------------------------------------------------------------
Revision History:
