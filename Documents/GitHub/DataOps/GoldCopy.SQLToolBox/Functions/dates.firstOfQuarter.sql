...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dates].[firstOfQuarter] 
(
  @date     DATETIME, 
  @quarters SMALLINT
)
...
--===== Autonomous use
 ...
 FROM dates.firstOfQuarter(@date, @quarters) f;

--===== Use against a table
 SELECT st.someDateCol, f.quarterStart
 FROM SomeTable st
 CROSS APPLY dates.firstOfQuarter(st.someDateCol, 5) f;
...
--==== Basic use ->
  ...
  SELECT f.quarterStart FROM dates.firstOfQuarter(@date,  0) f; -- first of this quarter
  SELECT f.quarterStart FROM dates.firstOfQuarter(@date,  1) f; -- first of next quarter
  SELECT f.quarterStart FROM dates.firstOfQuarter(@date, -1) f; -- first of last quarter
...
BEGIN
  ...
  FROM @table t
  CROSS APPLY dates.firstOfQuarter(t.someDate, 0) f;

  -- NEXT quarter
  SELECT q = 'next', t.someDate, f.quarterStart
  FROM @table t
  CROSS APPLY dates.firstOfQuarter(t.someDate, 1) f;

  -- 2 QUARTERS BACK
  SELECT q = '2 Back', t.someDate, f.quarterStart
  FROM @table t
  CROSS APPLY dates.firstOfQuarter(t.someDate, -2) f;
END;
