...
--===== Basic Syntax Example
 SELECT pr.NewString
 FROM samd.patReplace8K(@String,@Pattern,@Replace);

--===== Replace numeric characters with a "*"
 SELECT pr.NewString
 FROM samd.patReplace8K('My phone number is 555-2211','[0-9]','*') pr;
...
--==== Using againsts a table
 ...
 FROM @table t
 CROSS APPLY samd.patReplace8K(t.oldstring,'[0-9]','*') pr;
...
iTally(N) AS 
(
  SELECT TOP (LEN(ISNULL(@String,0))) CHECKSUM(ROW_NUMBER() OVER (ORDER BY (SELECT 1))) 
  ...
  CASE WHEN @string = '' THEN ''
      ...
      SELECT
        CASE
          ...
          THEN SUBSTRING(@String,N,1) ELSE @replace
        END
  FROM iTally
  FOR XML PATH(''), TYPE).value('text()[1]','varchar(800)')) END;
GO
