...
Syntax:
 ...
 FROM samd.patExtract(@string,@pattern) pe;

 --===== Against a table using APPLY
 SELECT t.someString, pe.ItemIndex, pe.ItemLength, pe.Item
 FROM samd.SomeTable t
 CROSS APPLY samd.patExtract(t.someString, @pattern) pe;
...
Developer Notes:
 1. Requires NGrams which can be found here: https://goo.gl/cXF9Gi

 2. patExtract does not return any rows on NULL or empty strings. Consider using 
 ...
 3. patExtract is not case sensitive; use a case sensitive collation for 
    case-sensitive comparisons

 4. patExtract is deterministic. For more about deterministic functions see:
    https://msdn.microsoft.com/en-us/library/ms178091.aspx

 5. patExtract performs substantially better with a parallel execution plan, often
