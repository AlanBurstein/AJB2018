...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [samd].[removeDupesExcept8K_2017](@string varchar(8000), @preserved varchar(50))
/*****************************************************************************************
Purpose:
...
--===== Autonomous use
 SELECT rd.newString
 FROM samd.removeDupesExcept8K_2017(@string, @preserved) rd;
--===== Against a table
 SELECT st.SomeColumn1, rd.newString
 FROM SomeTable st
 CROSS APPLY samd.removeDupesExcept8K_2017(st.SomeColumn1, @preserved) rd;
...
BEGIN
  ...
  --===== Remove all duplicate characters
    SELECT f.newString 
    FROM samd.removeDupesExcept8K_2017(@string,'') f; -- Returns: !a#b!
  
  --===== Remove all non-alphabetical duplicates
    SELECT f.newString 
    FROM samd.removeDupesExcept8K_2017(@string,'[a-z]') f; -- Returns: !aa#bb!
  
  --===== Remove only alphabetical duplicates
    SELECT f.newString 
    FROM samd.removeDupesExcept8K_2017(@string,'[^a-z]') f; -- Returns: !!!a###b!!!
END
---------------------------------------------------------------------------------------
Revision History:
 Rev 00 - 20171215 - Initial Creation - Alan Burstein
 Rev 02 - 20180615 - Updated to use NGrams8K - Alan Burstein
****************************************************************************************/
