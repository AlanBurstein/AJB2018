...
GO
CREATE NONCLUSTERED INDEX [nc_poc__calandarAB__CalYearNbr_monthNbr] ON [dbo].[CalendarAB] ([CalYearNbr], [MonthNbr], [DayOfMonthNbr]) INCLUDE ([DateID], [DateValue], [DayOfWeekNbr], [DayOfWeekTxtFull], [DayOfWeekTxtShort], [DayOfYearNbr], [IsEndOfMonth], [IsLeapYear], [ISOWeekNbr], [IsWeekend], [MonthTxtFull], [MonthTxtShort], [MonthYearTxt], [QuarterNbr], [WeekOfYearNbr], [YearQuarterTxt]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [uq_CalendarAB__DateValue] ON [dbo].[CalendarAB] ([DateValue], [IsWeekend]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [nc_lookup__calandarAB__CalYearNbr_monthNbr] ON [dbo].[CalendarAB] ([DayOfMonthNbr], [CalYearNbr], [MonthNbr]) ON [PRIMARY]
GO
