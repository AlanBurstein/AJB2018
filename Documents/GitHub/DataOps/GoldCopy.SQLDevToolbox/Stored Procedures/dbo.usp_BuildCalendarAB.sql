...
---------------------------------------------------------------------------------------
Revision History: 
 Rev 00 - 20160323 - Initial Development - Alan Burstein 
 Rev 01 - 20180608 - Removed the fiscal year, now using rangeAB for the tally table. 
                   - Fixed issue with end-of-month
                   - Added day column to index: nc_poc__calandarAB__CalYearNbr_monthNbr
                     to allow for the retrieval of one record per month lookups
										- Alan Burstein
****************************************************************************************/
BEGIN
...
CREATE NONCLUSTERED INDEX nc_poc__calandarAB__CalYearNbr_monthNbr
ON dbo.CalendarAB (CalYearNbr, monthNbr, DayOfMonthNbr)
INCLUDE (DateID,DateValue,DayOfYearNbr,QuarterNbr,MonthTxtFull,MonthTxtShort,
         WeekOfYearNbr,ISOWeekNbr,DayOfWeekNbr,DayOfWeekTxtFull,DayOfWeekTxtShort,
         MonthYearTxt,YearQuarterTxt,IsLeapYear,IsWeekend,IsEndOfMonth);

-- This index allows a fast, easy Year/Month lookup table
CREATE NONCLUSTERED INDEX nc_lookup__calandarAB__CalYearNbr_monthNbr
ON dbo.CalendarAB (DayOfMonthNbr, CalYearNbr, MonthNbr)

-- Usage:
/*
SELECT c.CalYearNbr, c.MonthNbr
FROM dbo.CalendarAB c --WITH (INDEX(nc_poc__calandarAB__CalYearNbr_monthNbr))
WHERE c.CalYearNbr    = 2010
AND   c.DayOfMonthNbr = 1;
*/;
