...
/*****************************************************************************************
Purpose:  !!! UPDATE TO MATCH THE 2012 VERSION
...
--===== Word-level Unigrams:
 ...
 ng.itemNumber,
 ng.item
 FROM WNGrams8K('One two three words',1) ng;
...
tokens(itemNumber, itemIndex, item, itemCount) AS
(
  SELECT N1.RN+1,
    COALESCE(NULLIF(N1.N,0),1), -- change to N then ORDER BY position to avoid a sort
   ...
   (LEN(@string) - LEN(REPLACE(@string, char(32),''))) - (@N-2)
  FROM delim N1
...
SELECT 
  ...
  t.item
FROM tokens t
WHERE @N > 0 AND t.itemNumber <= t.itemCount;
