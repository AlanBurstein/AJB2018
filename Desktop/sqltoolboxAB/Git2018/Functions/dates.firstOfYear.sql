...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dates].[firstOfYear] (@date datetime, @years smallint)
/*****************************************************************************************
[Purpose]:
...
[Author]:
 Alan Burstein
 Note: The idea for this function came from Lynn Pettis on SQLServerCentral.com. See:
...
--===== Autonomous use
 SELECT f.yearStart
 FROM dates.firstOfYear(@date, @years) f;

--===== Use against a table
 SELECT st.someDateCol, f.yearStart
 FROM SomeTable st
 CROSS APPLY dates.firstOfYear(st.someDateCol, 5) f;
...
--==== Basic use ->
  ...
  SELECT f.yearStart FROM dates.firstOfYear(@date,  0) AS f; -- first of this year
  SELECT f.yearStart FROM dates.firstOfYear(@date,  1) AS f; -- first of next year
  SELECT f.yearStart FROM dates.firstOfYear(@date, -1) AS f; -- first of last year
...
BEGIN
  ...
  FROM        @table AS t
  CROSS APPLY dates.firstOfYear(t.someDate, -1) AS f;
END;
