...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [samd].[patternSplit8K] (@string varchar(8000), @pattern varchar(50))
/****************************************************************************************
[Description]:
 ...
 a 1 is returned, for items that do not match a 0 is returned. Note this example: 
	...
	SELECT ps.* FROM samd.patternSplit8K('abc12345xyz', '[0-9]') ps;
...
[Syntax]:
 --===== Autonomous
  SELECT ps.itemNumber,
         ...
         ps.item,
         ps.isMatch 
  FROM   samd.patternSplit8K(@string,@searchString) AS ps;
 ...
 --===== Against a table using APPLY
  ...
  CROSS APPLY samd.patternSplit8K(string,@searchString) AS ps;

[Developer Notes]:
 1. Requires rangeAB which can be found here: https://goo.gl/Cb3S3y

 2. samd.patternSplit8K does not return any rows on NULL or empty strings. Consider using 
 ...
 3. samd.patternSplit8K is not case sensitive; use a case sensitive collation for 
   ...
   SELECT
     ...
     (
       SELECT 
         ...
         ELSE pe.item END+''
       FROM temp t2
       CROSS APPLY samd.patternSplit8K(t1.txt, '[() 0-9-]') pe
       WHERE t1.id = t2.id
...
SELECT itemNumber = ROW_NUMBER() OVER (ORDER BY patSplit.itemIndex), 
       patSplit.itemIndex,
       patSplit.itemLength,
       patSplit.item,
       patsplit.isMatch
...
(
  SELECT itemIndex  = 1, 
         itemLength = PATINDEX('%'+@pattern+'%', @string)-1,
         item       = SUBSTRING(@string,1,PATINDEX('%'+@pattern+'%', @string)-1), 
         isMatch    = CAST(0 AS BIT)
  WHERE  SUBSTRING(@string,1,1) NOT LIKE @pattern
  UNION ALL
  SELECT r.rn,
         itemLength = LEAD(r.rn,1,DATALENGTH(@string)+1) OVER (ORDER BY r.rn)-r.rn,
         item       = SUBSTRING(@string,r.rn,LEAD(r.rn,1,DATALENGTH(@string)+1) 
                        OVER (ORDER BY r.rn)-r.rn),
         isMatch    = ABS(t.p-2+1)
...
-- PREVIOUS VERSIONS: (BOTH A LITTLE FASTER BUT DIE WITH VARCHAR(8000))
;
-- Most Current:
/*
SELECT 
...
(
  ...
  FROM samd.NGrams8K(@string, 1) ng
  CROSS APPLY (VALUES(
