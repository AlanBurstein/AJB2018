...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dates].[firstOfWeek] (@date datetime, @weeks smallint)
/*****************************************************************************************
[Purpose]:
...
--===== Autonomous use
 SELECT f.weekStart
 FROM   dates.firstOfWeek(@date, @weeks) AS f;

--===== Use against a table
 SELECT      st.someDateCol, f.weekStart
 FROM        SomeTable AS st
 CROSS APPLY dates.firstOfWeek(st.someDateCol, 5) AS f;
...
--==== 1. Basic use
  ...
  SELECT f.weekStart FROM dates.firstOfWeek(@date,  0) f; -- start of this week
  SELECT f.weekStart FROM dates.firstOfWeek(@date,  1) f; -- start of next week
  SELECT f.weekStart FROM dates.firstOfWeek(@date, -1) f; -- start of last week
...
--==== 2. against a table
  ...
  FROM        @table AS t
  CROSS APPLY dates.firstOfWeek(t.someDate, -10) AS f;
