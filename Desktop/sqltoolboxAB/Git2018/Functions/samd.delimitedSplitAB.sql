...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [samd].[delimitedSplitAB]
(
...
--===== Autonomous
 ...
 FROM   samd.delimitedSplitAB(@string, @delimiter) AS ds;

--===== Against a table using APPLY
 SELECT t.someId, ds.itemNumber, ds.itemIndex, ds.itemLength, ds.item
 FROM        dbo.someTable AS t
 CROSS APPLY samd.delimitedSplitAB(t.someString, @delimiter) AS ds;
...
[Developer Notes]:
 ...
 2. delimitedSplitAB is not case sensitive
 3. delimitedSplitAB is deterministic. For more deterministic functions see:
...
--===== 2. Against a table
 ...
 SELECT      t.someId, ds.itemNumber, ds.itemIndex, ds.itemLength, ds.item
 FROM        @table AS t
 CROSS APPLY samd.delimitedSplitAB(t.someString, ',') AS ds;

-----------------------------------------------------------------------------------------
[Revision History]: 
 Rev 00 - 20181017 - Created - Alan Burstein
*****************************************************************************************/
SELECT
