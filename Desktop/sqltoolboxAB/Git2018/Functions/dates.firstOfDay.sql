...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dates].[firstOfDay] (@date datetime, @days smallint)
/*****************************************************************************************
[Purpose]:
...
--===== Autonomous use
 SELECT f.dayStart
 FROM   dates.firstOfDay(@date, @days) AS f;

--===== Use against a table
 SELECT     st.someDateCol, f.dayStart
 FROM        SomeTable AS st
 CROSS APPLY dates.firstOfDay(st.someDateCol, 5) AS f;
...
[Examples]:
--==== Basic use ->
  SELECT f.dayStart FROM dates.firstOfDay(getdate(),   0) f; -- start of today
  SELECT f.dayStart FROM dates.firstOfDay(getdate(),   1) f; -- start of the day tomorrow
  SELECT f.dayStart FROM dates.firstOfDay(getdate(), -10) f; -- start of the day 10 days ago
...
--==== against a table ->
  ...
  FROM @table t
  CROSS APPLY dates.firstOfDay(t.someDate, -10) f;
