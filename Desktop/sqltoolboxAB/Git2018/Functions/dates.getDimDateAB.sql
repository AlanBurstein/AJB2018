...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dates].[getDimDateAB]
(
  @startDate  DATE, 
  @endDate    DATE,
...
[Syntax]:
 SELECT d.* FROM dates.getDimDateAB(@startDate,@endDate) d;
...
[Developer Notes]:
 ...
 4. The only possible sorting on this function is against the seqNbr column; for 
    situations where your query would benefit from an index please consider using a 
    persisted calendar table (AKA DIM_DATE). When no indexes are necessary this function
    will outperform a persisted calendar table because it requires no space and doesn't 
    generate any reads, scans, lookups, etc. 
 5. To get a range of years you can just enter a year as the parameter like so:
      SELECT d.* FROM dates.getDimDateAB('2017','2019',0) d;
   
[Examples]:
 --===== 1. Get a datedim table between the dates 1/1/2000 and 3/1/2000:
 SELECT d.* FROM dates.getDimDateAB('1/1/2000','3/1/2000',0) d ORDER BY SeqNbr; -- ASC
 SELECT d.* FROM dates.getDimDateAB('1/1/2000','3/1/2000',1) d ORDER BY SeqNbr; -- DESC

 --===== 2. Same as Example 1 but filtered for weekdays only (assuming @@datefirst = 7):
 SELECT d.* 
 FROM dates.getDimDateAB('1/1/2000', '1/16/2000', 0) d
...
SELECT
  SeqNbr         = rn,
  DateID         = CAST(REPLACE(base.DateValue,'-','') AS int),
  DateValue      = base.DateValue,
  CalYearNbr     = YEAR(base.DateValue),
  DayOfYearNbr   = DATEPART(DAYOFYEAR,base.DateValue),
  QuarterNbr     = CAST(((DATEPART(M,base.DateValue)-1)/3)+1 AS tinyint),
  MonthNbr       = CAST(MONTH(base.DateValue) AS tinyint),
  MonthTxt       = DATENAME(M,base.DateValue),
  MonthTxt3      = LEFT(DATENAME(M,base.DateValue),3),
  DayOfMonthNbr  = CAST(DATEPART(D,base.DateValue) AS tinyint),
  WeekOfYearNbr  = CAST(DATEPART(WEEK,DateValue) AS tinyint),
  ISOWeekNbr     = CAST(DATEPART(ISO_WEEK,base.DateValue) AS tinyint),
  DayOfWeekNbr   = CAST(DATEPART(W,base.DateValue) AS tinyint),
  DayOfWeekTxt   = DATENAME(W,base.DateValue),
  DayOfWeekTxt3  = LEFT(DATENAME(W,base.DateValue),3),
  MonthYearTxt   = DATENAME(M,base.DateValue)+' '+
                     CAST(DATEPART(YEAR,base.DateValue) AS CHAR(4)),
  YearQuarterTxt = 'Y'+CAST(YEAR(base.DateValue) AS char(4))+' '+
                   'Q'+CAST(((DATEPART(M,base.DateValue)-1)/3)+1 AS char(1)),
  IsLeapYear     =
    CASE 
      WHEN (YEAR(base.DateValue) & 3 )   > 0 THEN 0 -- Anything with 1st or 2nd bits set is not a leap year
      WHEN (YEAR(base.DateValue) % 400 ) = 0 THEN 1 -- Anything divisible by 400 is a leap year
      WHEN (YEAR(base.DateValue) % 100 ) = 0 THEN 0 -- When year%100=0 and year%400<>0 then its not a leap year
      WHEN (YEAR(base.DateValue) % 4 )   = 0 THEN 1 -- When year%100<>0 and year%400<>0 and year%4=0 then its a leap year
      ELSE 0
    END,
  IsWeekend      = CASE WHEN DATENAME(WEEKDAY,base.DateValue) IN 
                    ('Saturday','Sunday') THEN 1 ELSE 0 END,
  IsEndOfMonth   = DATEDIFF(MM,DateValue, DATEADD(dd,1,DateValue))
FROM
( 
  SELECT 
    r.rn, 
    DateValue = DATEADD(dd,CASE @descending WHEN 0 THEN r.rn ELSE r.op END,@startDate)
  FROM dbo.rangeAB(1, SIGN(DATEDIFF(dd,@startDate,@endDate)) -- ensures that @endDate > @startDate
                      * DATEDIFF(dd,@startDate,@endDate)+1, 1, 0) r
) AS base;
GO
