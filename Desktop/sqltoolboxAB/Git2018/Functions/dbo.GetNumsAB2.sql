...
CREATE FUNCTION [dbo].[GetNumsAB2]
(
  @low  bigint, 
  @high bigint, 
  @gap  bigint, 
...
Developer Notes:
 1. The lowest and highest possible numbers returned are whatever is allowable by a 
    bigint. The function, however, returns no more than 100,544,625 rows (465^3). You can 
    add an additional cross join to L1 in the ITALLY for up to 46,753,250,625 rows. I 
    went with only three cross joins for a cleaner execution plan. 
 ...
 6. Although @gap is a bigint it must be a positive integer or the function will
    not return any rows.
 7. @high must be equal or higher than @low AND @gap must be greater than 0 OTHERWISE the function will return only NULLs.

 8. There is no performance penalty for sorting by rn ASC; there is a large performance 
    penalty for sorting in descending order WHEN @row1 = 1; WHEN @row1 = 0.... MERGE JOIN....
		
		To sort in descending order, first make sure
...
SELECT TOP (ABS((ISNULL(@high,0)-ISNULL(@low,0))/ISNULL(@gap,0)+ISNULL(@row1,1)))
  rn,
  op = ((@high-@low)/@gap+@row1)+@row1-rn,
  n1 = (rn-@row1)*@gap+@low,
  n2 = (rn-(@row1-1))*@gap+@low
FROM iTally
WHERE (@high+@low+@gap+@row1 IS NOT NULL AND @high >= @low AND @gap > 0)
AND rn <= ABS((ISNULL(@high,0)-ISNULL(@low,0))/ISNULL(@gap,0)+ISNULL(@row1,1))
